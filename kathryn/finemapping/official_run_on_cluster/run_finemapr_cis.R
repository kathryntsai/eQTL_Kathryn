...
args <- commandArgs(trailingOnly = T)   # enable reading factors from linux
# check for required num.rows argument
if (length(args) < 1)
  stop("Missing argument: num.chrs")
...
print(head(franke_cis_data))

devtools::load_all("/data/srlab/ktsai/finemapr")
print("successfully loaded finemapr")
  ...
  for(i in 1:length(chr1_cis_split)){
    ...
    tryCatch({
      ...
      plot(out_paintor, label_size = 3)
        ...
        cat("ERROR with ", this_gene$GeneSymbol[1], ":", conditionMessage(e), "\n")
      })
  }
