...
ggplot(all_snp_pwms_alt, aes(x=OtherPWM, y=AssessedPWM, color=cut(as.numeric(MotifScore), 6))) +
  ...
  labs(x="Ref Allele", y="Alt Allele") + geom_abline(color="blue", alpha=.8)
...
ggplot(all_snp_pwms_ref, aes(x=AssessedPWM, y=OtherPWM, color=cut(as.numeric(MotifScore), 6))) +
  ...
  labs(x="Alt Allele", y="Ref Allele") + geom_abline(color="blue", alpha=.8)
...
df2 <- melt(data.frame(all_snp_pwms_alt[order(all_snp_pwms_alt$'OtherPWM'), c("SNPWindow", "AssessedPWM", "OtherPWM")], id.vars='SNPWindow'))
