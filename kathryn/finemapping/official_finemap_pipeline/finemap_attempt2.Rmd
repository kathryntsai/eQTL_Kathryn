...
z1 <- read_zscore(paste0(my_path,"finemapping/abcd6_z.z"))

# devtools::load_all("/Users/kathryntsai/Documents/GitHub/finemapr_variani/R/finemap.R")
# zz <- read.table(paste0(my_path, "finemapping/abcd6_ld.ld", sep="\t", row.names = 1, header=T))
# fwrite(zz, "abcd6_ldlink.ld", sep="\t", row.names=F, col.names=F)
#ld1 <- read_ld(paste0(my_path, "finemapping/abcd6_ldlink.ld"))
...
devtools::load_all("/Users/kathryntsai/Documents/GitHub/finemapr")
ld1 <- read_ld(paste0(my_path, "finemapping/abcd6_ld.ld"))
...
print(out_finemap)
plot_ncausal(out_finemap, label_size = 3, grid_nrow=3)
plot_config(out_finemap, label_size = 3, top_rank=10)
plot_snp(out_finemap, label_size = 3)#, grid_ncol = 1)

# Extra
out_finemap[["config"]][["num_elem"]] <- unlist(lapply(str_split(out_finemap[["config"]][["config"]], ","), function(x) length(x)))
plot_config_nsnps.FinemaprFinemap(out_finemap, 2, label_size=1)
