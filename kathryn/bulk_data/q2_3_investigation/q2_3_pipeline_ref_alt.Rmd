...
if (ref_test_group_with_fasta$ref_Offset[i] >= (21 - nchar(tf_motif)) & ref_test_group_with_fasta$ref_Offset[i] < 21)
{
    ...
    colnames(allele_pwms) <- c("AssessedPWM", "Other PWM")
...
colnames(all_snp_pwms_ref) <- c("Num", "Index", "Base", "i", "SNPWindow", "Offset", "Sequence", "Motif Name", "Strand", "MotifScore", "Num1", "SNPPosInFasta", "Gene", "GeneSymbol", "GeneChr", "GenePos", "SNP", "SNPChr", "SNPPos", "AssessedAllele", "OtherAllele", "SNPWindowStart", "SNPWindowEnd", "fasta_seq_og", "fasta_seq_mod", colnames(allele_pwms))
...
colnames(all_snp_pwms_alt) <- c("Num", "Index", "Base", "i", "SNPWindow", "Offset", "Sequence", "Motif Name", "Strand", "MotifScore", "Num1", "SNPPosInFasta", "Gene", "GeneSymbol", "GeneChr", "GenePos", "SNP", "SNPChr", "SNPPos", "AssessedAllele", "OtherAllele", "SNPWindowStart", "SNPWindowEnd", "fasta_seq_og", "fasta_seq_mod", colnames(allele_pwms))
...
ggplot(all_snp_pwms_ref, aes(x=as.numeric(Num), fill=cut(as.numeric(i), 3))) + 
...
ggplot(all_snp_pwms_alt, aes(x=as.numeric(Num), fill=cut(as.numeric(i), 3))) + 
...
```{r further_exploration_alt}

df2 <- melt(data.frame(all_snp_pwms_alt[, c("SNPWindow", "AssessedPWM", "Other PWM")], id.vars='SNPWindow'))
...
df2 <- melt(data.frame(all_snp_pwms_ref[, c("SNPWindow", "AssessedPWM", "Other PWM")], id.vars='SNPWindow'))
ggplot(df2, aes(x=SNPWindow, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge')
